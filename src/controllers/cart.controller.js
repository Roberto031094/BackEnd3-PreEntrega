...
export class CartController {
  ...
  async addProductToCart(req, res) {
    try {
      ...
      if (!product) return res.status(404).json({ status: "Error", msg: `Producto no encontrado: ID ${pid}` });

      const cart = await cartService.addProductToCart(cid, pid);
      if (!cart) return res.status(404).json({ status: "Error", msg: `Carrito no encontrado: ID ${cid}` });
  ...
  async purchaseCart(req, res) {
    try {
      ...
      if (total === 0) return res.status(400).json({ status: "Error", msg: "No hay stock suficiente para comprar los productos" });
